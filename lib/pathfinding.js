...
var Pathfinding = /** @class */ (function () {
    ...
    Pathfinding.findPath = function (grid, startX, startY, endX, endY, costThreshold) {
        return __awaiter(this, void 0, void 0, function () {
            var instance, startNode, node, path, parent;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        instance = new search_1.default({
                            startX: startX, startY: startY, endX: endX, endY: endY, costThreshold: costThreshold
                        });
                        startNode = Pathfinding.coordinateToNode(instance, null, startX, startY, 0);
                        instance.push(startNode);
                        return [4 /*yield*/, Pathfinding.calculate(grid, instance)];
                    case 1:
                        _a.sent();
                        node = instance.nodeQueue.pop();
                        if (node) {
                            path = [];
                            path.push(new coord_1.default(node.x, node.y));
                            parent = node.parent;
                            while (parent) {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        startNode = Pathfinding.coordinateToNode(instance, null, x, y, 0);
                        instance.push(startNode);
                        return [4 /*yield*/, Pathfinding.calculate(grid, instance)];
                    case 1:
                        _a.sent();
                        nodes = [];
                        instance.nodeCache.forEach(function (map, y) {
    ...
    Pathfinding.calculate = function (grid, instance) {
        while (true) {
            //fully traversed
            if (instance.nodeQueue.size() === 0) {
                return instance;
            }
            var node = instance.nodeQueue.peek();
            //path found
    ...
    Pathfinding.getCoordCost = function (grid, x, y) {
        var extraCost = grid.extraCosts.has(y) && grid.extraCosts.get(y).get(x);
        return extraCost || grid.costs.get(grid.tiles[y][x]);
    };
    Pathfinding.isCoordWalkable = function (grid, x, y) {
        var unwalkable = grid.unwalkableCoords.has(y) &&
            grid.unwalkableCoords.get(y).get(x);
        return !unwalkable && grid.walkableTiles.indexOf(grid.tiles[y][x]) !== -1;
    };
    Pathfinding.checkAdjacentNode = function (grid, instance, sourceNode, x, y) {
        var adjacentX = sourceNode.x + x;
        var adjacentY = sourceNode.y + y;
        var adjacentCost = Pathfinding.getCoordCost(grid, adjacentX, adjacentY);
        if (Pathfinding.isCoordWalkable(grid, adjacentX, adjacentY) &&
            Pathfinding.canAfford(sourceNode, adjacentCost, instance.costThreshold)) {
            var adjacentNode = Pathfinding.coordinateToNode(instance, sourceNode, adjacentX, adjacentY, adjacentCost);
    ...
    Pathfinding.coordinateToNode = function (instance, parent, x, y, cost) {
        if (instance.nodeCache.has(y)) {
            ...
            instance.nodeCache.set(y, new Map());
        }
